...
4. `apply_data` 只写白名单路径，批量校验后一次 Git commit；页面只读已提交 revision。提交失败只恢复本请求触及的路径。
5. 图片通过 assistant-ui 附件适配上传；服务端真实解码 JPEG/PNG/静态 WebP，限制 10 MiB/2000 万像素，旋转、去元数据、重编码并保存哈希。模型接收 Base64 图片内容，不接收私有 URL 或路径。
6. Pi SDK 使用当前锁定包 `@earendil-works/pi-coding-agent@0.84.4`。每个账号独立会话目录；禁用内置宿主工具与可写目录自动发现，只注册本应用工具。
...
9. UI Agent 仅写 `web/src` 与 `web/public`。候选使用 Git worktree；配置固定镜像后，检查在 rootless Podman 中以断网、无密钥、无个人数据、受限资源方式运行。发布绑定源码哈希和产物哈希；`/ops` 独立提供回滚。
...
7. 使用 `deploy/Containerfile.ui-check` 构建固定 UI 检查镜像，记录不可变 digest，再设置 `UI_SANDBOX_IMAGE`。若 rootless 沙箱不可用，保持 UI 编辑关闭，不得在宿主机运行候选代码。
...
- Podman/固定镜像缺失：普通 Agent 继续；UI 代码执行关闭。
