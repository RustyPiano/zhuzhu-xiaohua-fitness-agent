...
4. 每轮从已部署 app revision/data main 创建独立副本和只读 inbox。容器内 `.git` 只读；宿主 Finalizer 按文件内容找出变化，将允许文件复制到宿主新建的可信 worktree，再校验完整 TypeBox Schema、人物/日期、符号链接和附件来源，覆盖新记录的操作身份后一次 fast-forward commit。
...
9. Agent 可在隔离 app 副本中检视全仓，Finalizer 只复制 `web/src` 与 `web/public` 到可信 Git worktree，其他路径不可发布。检查在 rootless Podman 中以断网、无密钥、无个人数据、受限资源方式运行。候选位于预览 origin 的 `/candidates/<job-id>/`；发布和 `/ops` 回滚整体绑定源码 revision、产物路径与哈希。
...
- 服务重启：登录需要重新进行；若数据 commit 已含对应 `Request-Id`，启动时补回保存回执，否则运行中的请求标为 `interrupted`，不会自动重放副作用。新 Pi 会话 generation 持久化，不会因重启续接旧会话。
