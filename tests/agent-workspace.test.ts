...
describe('isolated Agent workspace finalizer', () => {
  ...
  it('skips the finalizer worktree when candidate data is clean', async () => {
    ...
    await expect(workspaceModule.finalizeDataWorkspace(workspace, 'zhuzhu', 'request-12345678', [])).resolves.toBeNull();
    await expect(readdir(path.join(process.env.RUNTIME_DIR, 'finalizers'))).rejects.toMatchObject({ code: 'ENOENT' });
  });
