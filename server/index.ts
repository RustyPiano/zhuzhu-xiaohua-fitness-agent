...
app.get('/api/bootstrap', async (c) => {
  ...
  const agentConfigured = modelConfigured && Boolean(config.agentSandboxImage);
  return c.json({
    actor: c.get('actor'), timezone: config.timezone, today: businessDate(), app_version: '1.0.0',
    agent: { configured: agentConfigured, reason: !modelConfigured ? '未配置支持图片和工具调用的模型' : config.agentSandboxImage ? null : '未配置 AGENT_SANDBOX_IMAGE' },
    ...
    ui_editing: { configured: agentConfigured && Boolean(config.uiSandboxImage), reason: !agentConfigured ? '当前 Agent 不可用' : config.uiSandboxImage ? null : '未配置固定 rootless Podman 构建镜像' },
  });
});
