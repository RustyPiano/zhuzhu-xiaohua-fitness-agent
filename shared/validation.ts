...
const exercise = Type.Object({ exercise_id: id, name: text(200), equipment: nullableText(200), sets: nullableNumber, reps: nullableText(100), load: nullableNumber, load_unit: nullableText(100), rest_seconds: nullableNumber, notes: stringList }, { additionalProperties: false });
...
const looseRecord = Type.Record(Type.String({ maxLength: 100 }), Type.Unknown(), { maxProperties: 40 });
const log = Type.Object({ schema_version: Type.Literal(1), date, person_id: person, plan_revision: nullableText(100), nutrition_status: Type.Union([Type.Literal('unlogged'), Type.Literal('partial'), Type.Literal('complete')]), meals: Type.Array(mealLog, { maxItems: 100 }), training_status: Type.Union([Type.Literal('unlogged'), Type.Literal('partial'), Type.Literal('complete')]), sets: Type.Array(setLog, { maxItems: 500 }), cardio: Type.Array(looseRecord, { maxItems: 100 }), measurements: Type.Array(looseRecord, { maxItems: 100 }), notes: stringList }, { additionalProperties: false });
...
export function validateBusinessJson(relativePath: string, value: unknown): void {
  if (relativePath.startsWith('people/')) check(profile, value, relativePath);
    ...
    check(log, value, relativePath); const typed = value as any; unique(typed.meals.map((item: any) => item.id), 'meals'); unique(typed.sets.map((item: any) => item.id), 'sets'); for (const meal of typed.meals) unique(meal.items.map((item: any) => item.id), `${meal.id}.items`);
